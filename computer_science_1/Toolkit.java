/*
 * Author: Prof. David Kramer. 
 * Provided to students for use in numerous class projects.
 */

// File Toolkit, Version 04 - 2014-07-29
import java.io.*;                   // For the PrintWriter file
import java.text.DecimalFormat;     // For the padding methods
import java.util.Scanner;           // For the partiallyFillArray method
import java.text.SimpleDateFormat;  // For the getCurrentTime method
import java.util.Calendar;          // For the getCurrentTime method
import java.util.Date;              // For the getCurrentDate method

public class Toolkit {

/** File Toolkit - a class with general tools, version nn
...
*/
   ...
   final public char OUTPUT_MODE_DEFAULT = OUTPUT_MODE_SCREEN; // Default
                                                  // Default output mode
   ...
   */
   public Toolkit(char outMode, FileWriter outputFileName) {
      ...
      setOutputMode(outMode);     // Establish the outputMode for this instance
   } // End cpmstructor with two arguments
   ...
   /** Method setOutputMode
       A Constructor or a user can change the outputMode at any time
   */
  ...
  */
   ...
   public void outputLine(String theLine) {
      ...
      if (outputMode == OUTPUT_MODE_FILE || outputMode == OUTPUT_MODE_BOTH)
      {
         if (outputFile != null) { // i.e., a file has been defined 
            ...
            System.out.println("File not set: "  + outputMe);
   ...
   */    
   ...
   public static String padNumber(double number, int width, 
                        String mask, String padLeft, String padRight) {
   ...
   */    
   ...
   /** partiallyFillArray
       Fill an integer array up to and including its length
       ...
       Output
         @return  nRead, the number of input records read
       ...
       Note       We calculate the number of rows left over after the array is
                  ...
                  input file, the left overs will be 5
   */
   ...
   public static int partiallyFillArray(Scanner inp, int[] array) {
      ...
      String inputLine;          // To flush the input file when nRead > len
      ...
      while (inp.hasNext()) {
         ...
         if (nRead < len) {
            ...
            inputLine = inp.nextLine();     // Flush unstored data
         } // End if
   ...
   */    
   ...
   public static String rightPad(double number, int width, 
                                 String mask, String padding) {
  ...
  */
   ...
   */
   ...
   public static void selectionSort(int[] array, int len) {
    ...
    int temp; 		// Temporarray storage while swapping array elements
   ...
   */
   ...
   public static void selectionSortWithIndex(int[] array, int[] index, 
                                             int len) {
   ...
   /**
      ...
      get moved around. This method does NOT use indexes.
   
      The parameters are:
      
      ary - a String array
      numbers - a 2-dimensional array of real numbers
      len - the number of possibly partially-filled elements in the array
            (i.e., it might be < ary.length)
      column - the column # in the numbers array to sort by, or -1 if we
               are sorting by String. For example, if column = 2, it means
               we are sorting by column #2 which is the 3rd column
   */
   public static void selectionSortStringWithNumbers(String[] ary,
                                 double[][] numbers, int len, int column) {

    /*
      The next two lines of code are to establish an array that can contain
